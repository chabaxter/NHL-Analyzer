...
def shooting_data_visual(first_year=2000, last_year=2020, season_type=2):
    shooting_data_seasons = []

    try:
        f = open(str(first_year) + '-' + str(last_year) + '_' + str(season_type) + '_' + "shooting.json", "r")
        shooting_data_seasons = json.load(f)
    except FileExistsError as e:
        for year in range(first_year, last_year + 1):
            shooting_plays_by_game = NHL_API.for_season(year, season_type, NHL_API.get_shooting_plays)
            shooting_data_seasons.append(shooting_plays_by_game)

        f = open(str(first_year)+'-'+str(last_year)+'_'+str(season_type)+'_'+"shooting.json", "w")
